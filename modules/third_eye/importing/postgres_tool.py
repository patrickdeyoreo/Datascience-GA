...
class PostgresTool(object):
    ...
    def query(self, queries):
        results = self._query(queries, list())
        return results if len(results) > 1 else results[0]
    
    
    def _query(self, queries, results):
        if type(queries) == list:
            for q in queries:
                self._query(q, results)
        elif type(queries) == str:
            results.append(self._pgcc.query(queries))
        else:
            raise ValueError("Argument 'queries' must be a string or a " +
